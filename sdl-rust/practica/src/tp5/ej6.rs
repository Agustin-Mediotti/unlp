...
const FILE_NAME: &str = "src/tp5/archivo_blockchain.json";
        ...
        if let Some(balance) = self.balances.get_mut(&user_id) {
            ...
            if self.usuarios.get(&user_id).is_some_and(|u| u.validado) {
                ...
                if balance.balance_fiat >= monto_fiat {
                    ...
                    // Guardo en archivo de persistencia
                    self.escribir_json().expect("error escribiendo el archivo");
                }
            }
        }
